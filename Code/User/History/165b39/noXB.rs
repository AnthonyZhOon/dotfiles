...
pub fn process(_input: &str) -> miette::Result<String, AocError> {
    ...
    while left != right {
        dist += 1;
        left = expand(&grid, left).iter().find(|&x| x.is_some_and(|x| x != (start_row, start_column))).unwrap().unwrap();
        right = expand(&grid, right).iter().find(|&x| x.is_some_and(|x| x != (start_row, start_column))).unwrap().unwrap();
    }
    Ok(dist.to_string())
}
