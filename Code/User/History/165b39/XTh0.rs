...
pub fn process(_input: &str) -> miette::Result<String, AocError> {
    ...
    let (mut prev_left, mut prev_right) = ((start_column))
    let mut dist = 1;
    while left != right {
        dist += 1;
        left = expand(&grid, left)
            .iter()
            .find(|&x| x.is_some_and(|x| x != (start_row, start_column) && x != left))
            .unwrap()
            .unwrap();
        right = expand(&grid, right)
            .iter()
            .find(|&x| x.is_some_and(|x| x != (start_row, start_column) && x != right))
            .unwrap()
            .unwrap();
    }
    Ok(dist.to_string())
