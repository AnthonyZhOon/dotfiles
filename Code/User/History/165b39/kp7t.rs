...
pub fn process(_input: &str) -> miette::Result<String, AocError> {
    ...
    .iter()
    .filter_map(|&x| if expand(&grid, x).into_iter().any(|y|y==Some((start_row,start_column))) {Some(x)} else {None} )
    // .inspect(|x| println!("{x:?}"))
    .collect_tuple()
    ...
    while left != right {
        dist += 1;
        left = expand(&grid, left).iter().find(|&x| x.is_some_and(|x| x != (start_row, start_column) && x != left)).unwrap().unwrap();
        right = expand(&grid, right).iter().find(|&x| x.is_some_and(|x| x != (start_row, start_column) && x != right)).unwrap().unwrap();
    }
    Ok(dist.to_string())
}
