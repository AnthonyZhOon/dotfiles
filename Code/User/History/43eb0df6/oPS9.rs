...
impl<'a> std::fmt::Display for ResponseStatus<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let x = match self {
            ResponseStatus::Ok200(_) => "HTTP/1.1 200 OK",
            ResponseStatus::Bad404 => "HTTP/1.1 404 NOT FOUND",
            ResponseStatus::Bad400 => "HTTP/1.1 400 BAD REQUEST",
            ResponseStatus::Fail500 => "HTTP/1.1 500 INTERNAL SERVER ERROR"
        };
        write!(f, "{x}")
    }
}
