...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while !success(&current) {
        ...
        match guess_astar {
            Some((path, _cost)) => save_result(Some(path), guess.clone(), start, goal, "astar.png", format!("Guessed {steps}"))?,
            None => save_result(None, guess.clone(), start, goal, "astar.png", "Guessed path (Impossible)")?
        }
        steps += 1;
    }
