...
pub fn process(_input: &str) -> miette::Result<String, AocError> {
    ...
    let times: Vec<f32> = time
        .split_once(':')
        .unwrap()
        .1
        .split_ascii_whitespace()
        .map(|x| x.parse().expect("Failed to parse times"))
        .collect();
    let dists: Vec<f32> = dist
        .split_once(':')
        .unwrap()
        .1
        .split_ascii_whitespace()
    ...
    let ways: u32 = times
        .iter()
        .zip(dists)
        .map(|(&time, dist)| feasible_span(time, dist))
        .product();
    Ok(ways.to_string())
}
...
fn feasible_span(time: f32, dist: f32) -> u32 {
    ...
    let cleaned: [f32; 2] = [(centre - x).ceil(), (centre + x).floor()];
