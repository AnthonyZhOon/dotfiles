...
pub fn process(_input: &str) -> miette::Result<String, AocError> {
    ...
    let dists: Vec<u32> = dist
        ...
        .map(|x| x.parse::<u32>().expect("Failed to parse times"))
        .collect();
    let ways = times.iter().zip(dists).map(|(&time, dist)| feasible_span(time as f32, dist as f32)).fold(1, |acc, x| acc*x).to_string();
}
...
fn feasible_span(time: f32, dist: f32) -> u32 {
    ...
    let cleaned: [f32; 2] = [roots[0].ceil(), roots[1].floor()];
