...
pub fn process(
    ...
    let mut graph: HashMap<Bytes<'_>, [Bytes<'_>; 2]> = HashMap::new();
    edges.split('\n').map(|line| {
        line.split_once(" = ").map(|(from, to)| {
            (from.bytes(),
            to.trim_start_matches("(").trim_end_matches(")").split_once(", ").map(|(l, r)| (l.bytes(), r.bytes())).expect("failed to split {to} tuple"))
