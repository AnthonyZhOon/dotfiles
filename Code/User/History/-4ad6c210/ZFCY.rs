...
pub fn process(
    ...
    let mut graph: HashMap<Chars<'_>, [Chars<'_>; 2]> = HashMap::new();
    edges.split('\n').map(|line| {
        line.split_once(" = ").map(|(from, to)| {
            (from.chars(),
            to.trim_start_matches("(").trim_end_matches(")").split_once(", ").map(|(l, r)| (l.chars(), r.chars())).expect("failed to split {to} tuple"))
